 ...
 * 2. Injects a quiet build-label line if the page has not already
 *    rendered one (element id "demo-banner").
 */
(function () {
  'use strict';

  window.DEMO_BUILD = 'tier1-33';
  ...
  function injectBanner() {
    ...
    /* One quiet line in normal document flow: the build label and nothing
     * else. NOT sticky, NOT fixed, no z-index, so it can never sit over the
     * Generator's own controls. The dev clones also say they are dev. */
    b.textContent = (devCloneRoot() ? 'DEV TEST ENVIRONMENT \u00b7 designCentral-dev \u00b7 Not Production \u00b7 ' : '')
      + 'build ' + (window.DEMO_BUILD || '');
     ...
     * with nothing overlapping and no gap. Presentation only. */
    if (devBanner) {
